Fortis.MotionManager = {
    ...
    update(delta) {
        for (id in this.list) {
            console.log(Fortis.MotionManager.list[id])
            Fortis.MotionManager.list[id]["elapsedTime"] += delta;
            let finish = false;
            if(Fortis.MotionManager.list[id]["elapsedTime"]>= Fortis.MotionManager.list[id]["time"])finish = true;console.log(finish);
            switch (Fortis.MotionManager.list[id]["type"]) {
                ...
                case "scale":
                    Fortis.MotionManager.list[id]["now"] += delta * Fortis.MotionManager.list[id]["difference"] / Fortis.MotionManager.list[id]["time"];
                    Fortis.MotionManager.list[id]["target"].scale.x = Fortis.MotionManager.list[id]["target"].scale.x = Fortis.Manager[id]["now"];
                    break;
            }
        }
    },
